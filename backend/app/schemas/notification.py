...
class NotificationBase(BaseModel):
    ...
    data: Optional[Dict[str, str]] = Field(None, description="Additional notification data")
...
class NotificationSendBatch(BaseModel):
    ...
    notifications: List[NotificationSend] = Field(..., description="List of notifications to send")
...
class NotificationPreferences(BaseModel):
    ...
    challenge_notifications: bool = Field(default=True, description="Enable challenge notifications")
    match_notifications: bool = Field(default=True, description="Enable match result notifications")
    general_notifications: bool = Field(default=True, description="Enable general notifications")
    push_enabled: bool = Field(default=True, description="Enable push notifications")
    email_enabled: bool = Field(default=False, description="Enable email notifications")
    quiet_hours_start: Optional[str] = Field(None, regex="^([01]?[0-9]|2[0-3]):[0-5][0-9]$", description="Quiet hours start time (HH:MM)")
    quiet_hours_end: Optional[str] = Field(None, regex="^([01]?[0-9]|2[0-3]):[0-5][0-9]$", description="Quiet hours end time (HH:MM)")
...
class NotificationHistoryList(BaseModel):
    ...
    notifications: List[NotificationHistory] = Field(..., description="List of notifications")
...
class NotificationStats(BaseModel):
    ...
    delivery_rate: float = Field(..., ge=0.0, le=1.0, description="Delivery rate percentage")
