...
class FirebaseService:
    ...
    def update_document(
        ...
        try:
            ...
            logger.error(
                f"Failed to update document {document_id} in {collection_name}: {e}"
            )
            return False
    ...
    def delete_document(self, collection_name: str, document_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete document {document_id} from {collection_name}: {e}"
            )
            return False
    ...
    async def query_documents_multiple(
        self, collection_name: str, filters: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.error(
                f"Failed to query documents with multiple filters in {collection_name}: {e}"
            )
            raise
